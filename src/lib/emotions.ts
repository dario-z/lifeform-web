...
async function requestModelSignals(
  ...
  const prompt = [
    ...
    '- Be conservative. Use action none by default.',
    '- For autonomous Key Memory proposals, use create or update only for a stable collaboration preference, standing communication preference, recurring workflow constraint, durable formatting/language/tone preference, or an already-established fact that is clearly essential for future assistance.',
    '- A project, workstream, sprint, ongoing exploration, hobby, role, relationship, personal detail, event, plan, temporary situation or topic mentioned in chat is NOT automatically a Key Memory merely because it might persist. Ongoing work belongs to an Active Thread; durable directions belong to Goals; ordinary personal information usually needs no proposal.',
    '- Do not create an autonomous Key Memory from a generic project name, a personal anecdote, a job detail, a temporary plan, a one-off decision, a current issue, or a topic that has appeared only in the current exchange.',
    '- Use category long_term_goal only for a small durable direction that can matter across future conversations. Never use it for a one-off task, appointment, generic project name or vague wish.',
    '- Use category lifeform_belief only for a concise, tentative perspective genuinely developed by the Lifeform about itself, its relationship or the world. Never use it for facts about the user or objective truth.',
    ...
    '- Goals and beliefs must be expressed as clearly durable statements. Do not infer them merely because a topic was mentioned once.',
    ...
    '- Only use create or update when importance is at least 85.',
